...
test('#014: Copy list feature from menu (My Lists page)',{
  ...
  await page.getByLabel('New list name').fill('copy t list 1');
  await page.getByLabel('Please select the source list').click();
  await page.getByRole('option', { name: 'test list 1 (3 entries)' }).click();
  await page.getByRole('button', { name: 'OK' }).click();
  await expect(page.getByRole('link', { name: 'copy t list 1 (linked)' })).toBeVisible();
  await page.getByRole('link', { name: 'copy t list 1 (linked)' }).click();
...
test('#046: Start BrightChat button from List Details page',{
  tag: ['@Messages'],
  annotation: [
    { type: '!!!Test is currently being skipped as this feature is currently bugged on this account.', description: ''},
...
/*
  ...
  //test #038 test msg at 2024-6-13_6-40
  await page.locator('div').filter({ hasText: /^My Lists$/ }).click();
  await page.getByRole('button', { name: 'ryan test' }).click();
